...
# Найти и заменить
async def response_search_replace_img_model(
    search_prompt, replace_prompt: str, img, telegram_id: int, db: DB
):
    # Цена одного запроса в кредитах
    price = 4
    key_row = await db.select_valid_key(price)
...
async def response_upscale_img_model(
    img, prompt: str, telegram_id: int, db: DB
) -> tuple[str, str | None]:
    # Цена одного запроса в кредитах
    price = 25
    key_row = await db.select_valid_key(price)
...
async def response_upscale_img_result(
    ...
    try:
        for i in range(1, 25):
            ...
            if response.status_code == 202:
                ...
                callback.message.bot.send_chat_action(
...
# Создать видео
async def response_image_to_video_model(
    img, telegram_id: int, db: DB
) -> tuple[str, str | None]:
    # Цена одного запроса в кредитах
    price = 20
    key_row = await db.select_valid_key(price)
...
async def response_image_to_video_result(
    callback: CallbackQuery,
    manager: BaseDialogManager,
    telegram_id: int,
    key: str,
        ...
        for i in range(1, 25):
            ...
            if response.status_code == 202:
                ...
                await callback.message.bot.send_chat_action(
                    callback.message.chat.id, action="upload_video"
                )
