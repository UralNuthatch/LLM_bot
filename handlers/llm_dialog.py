...
async def get_llms(
    dialog_manager: DialogManager, i18n: TranslatorRunner, db: DB, **kwargs
):
...
async def set_llm(
    ...
    try:
        ...
        llm: Record = await db.set_llm_to_user(
            telegram_id=callback.message.chat.id,
            telegram_username=callback.from_user.username,
            telegram_name=callback.from_user.first_name,
            llm_id=int(item_id),
        )
        if callback.message.chat.type == "private":
            msg = await callback.message.answer(
                f'{llm[0].get("img")} {llm[0].get("name")}\n{i18n.send.request()}'
            )
        else:
